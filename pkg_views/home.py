        ...
        if st.button('삼행시 만들기'):
            ...
            with pbar:
                ...
                if not img_url:
                    ...
                    pbar.change_progress('이미지 생성에 실패했습니다. 기본 이미지를 사용합니다.', 20, 40)
                else:
                    pbar.change_progress('이미지 생성이 완료되었습니다.', 20, 40)

                pbar.change_progress('오디오를 생성 중 입니다.', 40, 50)
                synthesize_speech(content, filename='temp/' + file_name + '.wav', ssml=True)
                pbar.change_progress('오디오 생성이 완료되었습니다.', 50, 60)

                pbar.change_progress('데이터를 저장 중 입니다.', 60, 70)
                ...
                pbar.change_progress('결과를 출력 하는 중 입니다.', 70, 80)
                st.title(user_input)
